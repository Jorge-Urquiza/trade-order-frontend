...
import { UpdateTradeOrderDto } from '../../models/trade-order.dto';
import {
  TradeOrder,
  TradeOrderStatus,
  TradeOrderType,
...
@Component({
  ...
  styleUrl: './trades-edit.component.scss',
  standalone: false,
})
export class TradesEditComponent implements OnDestroy {
  ...
  public readonly statusOptions: SelectOption<TradeOrderStatus>[] = [
  ...
  public readonly form = this.formBuilder.group({
    pair: this.formBuilder.control<TradePair | null>(null, [Validators.required]),
    side: this.formBuilder.control<TradeSide | null>(null, [Validators.required]),
    type: this.formBuilder.control<TradeOrderType | null>(null, [Validators.required]),
    ...
    status: this.formBuilder.control<TradeOrderStatus | null>(null, [Validators.required]),
  });
  ...
  public constructor(
    ...
    @Optional() config: DynamicDialogConfig<{ trade: TradeOrder }>,
  ) {
    this.trade = config?.data?.trade ?? null;
    if (!this.trade) {
      this.messageService.add({
        severity: 'error',
        summary: 'Invalid trade',
        detail: 'Trade data was not provided to edit dialog.',
      });
  ...
  public onSubmit(): void {
    ...
    this.tradeOrdersService
      .updateTradeOrder(this.trade.id, payload)
      ...
      .subscribe({
        next: () => {
          this.messageService.add({
            ...
            detail: 'Trade order was updated successfully.',
          });
  ...
  private buildPayload(): UpdateTradeOrderDto {
    ...
    return {
      ...
      status: raw.status as TradeOrderStatus,
    };
  }
  ...
  private handleUpdateError(error: HttpErrorResponse): void {
    ...
    if (messages.length === 0) {
      this.messageService.add({
        severity: 'error',
        summary: 'Could not update trade',
        detail: 'Unexpected error while updating trade order.',
      });
      return;
    }
